...
// Visitor class to implement `Polynomial(const Expression& e, const Variables&
// indeterminates)` constructor which decomposes an expression e
...
class DecomposePolynomialVisitor {
 ...
 private:
  ...
  // Helper method for VisitAddition. Add coeff * m to map.
  void Add(const Expression& coeff, const Monomial& m,
           Polynomial::MapType* const map) const {
    auto it = map->find(m);
    if (it != map->end()) {
      // m ∈ dom(map)
...
Polynomial& Polynomial::operator+=(const Polynomial& p) {
  ...
  for (const pair<Monomial, Expression>& p : p.monomial_to_coefficient_map_) {
    ...
    if (it == monomial_to_coefficient_map_.end()) {
      ...
      monomial_to_coefficient_map_.emplace_hint(it, m, coeff);
    } else {
      // The entry, coeff * m in p, exists in this polynomial.
      if (is_zero(it->second + coeff)) {
        // Cancel the entry in the polynomial if needed.
        monomial_to_coefficient_map_.erase(it);
      } else {
...
Polynomial& Polynomial::operator*=(const Polynomial& p) {
  ...
  *this = Polynomial{this->ToExpression() * p.ToExpression(), indeterminates_};
  return *this;
}

Polynomial& Polynomial::operator*=(const Monomial& m) {
  // Updates indeterminates.
  for (const pair<Variable, int>& p : m.get_powers()) {
    indeterminates_ += p.first;
  }
...
bool Polynomial::EqualTo(const Polynomial& p) const {
  ...
  return indeterminates_ == p.indeterminates() &&
         monomial_to_coefficient_map_ == p.monomial_to_coefficient_map();
}
...
Polynomial& Polynomial::Add(const double coeff, const Monomial& m) {
  ...
  if (it == monomial_to_coefficient_map_.end()) {
    ...
    monomial_to_coefficient_map_.emplace_hint(it, m, coeff);
    // Update indeterminates_.
    for (const pair<Variable, int>& p : m.get_powers()) {
      indeterminates_ += p.first;
    }
  } else {
    // m is already in the polynomial.
    if (is_zero(it->second + coeff)) {
      // Cancels `m` from the map if needed.
      monomial_to_coefficient_map_.erase(it);
    } else {
      // Otherwise, increase its corresponding coefficient.
      it->second += coeff;
    }
  }
  return *this;
...
void Polynomial::CheckInvariant() const {
  for (const pair<Monomial, Expression>& p : monomial_to_coefficient_map_) {
    const Monomial& m{p.first};
    if (!m.ToExpression().GetVariables().IsSubsetOf(indeterminates_)) {
      ostringstream oss;
      oss << "Monomial " << m
          << " includes an indeterminate which is not a member of "
          << indeterminates_ << ".";
      throw runtime_error(oss.str());
    }
    const Expression& coeff{p.second};
    for (const Variable& var : coeff.GetVariables()) {
      if (indeterminates_.include(var)) {
        ostringstream oss;
        oss << "Indeterminate " << var
            << " is included in a coefficient: " << coeff << ".";
        throw runtime_error(oss.str());
      }
    }
  }
