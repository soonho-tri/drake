...
//     auto velocity =
//         dynamic_cast<const
//         FrameVelocity<double>*>(output_->get_vector_data(2));
...
//   // Checks that the velocity output has the correct values for the state
//   that
//   // InitializeNonzeroState sets.
...
//   default_params.max_abs_steering_angle(); const double max_abs_curvature =
//   std::tan(max_abs_steering_angle) / wheelbase; const double max_acceleration
//   = default_params.max_acceleration();
...
//   // A non-zero steering_angle turns in the same direction.  We'd like to
//   turn
...
//   // can remove this test once the tanh in the implementation goes away, to
//   be
...
//     // TODO(jwnimmer-tri) We should have a framework way to just say "make
//     the
//     // entire context symbolic variables (vs zero)" that is reusable for any
//     // consumer of the framework.
//     auto input_value =
//     std::make_unique<DrivingCommand<symbolic::Expression>>();
//     other_context->FixInputPort(0, std::move(input_value));
