...
GTEST_TEST(SymbolicCodeGenTest, Test1) {
  const Variable x{"x"};
  const Variable y{"y"};
  ...
  const double param[2] = {1 /* x */, 2 /* y */};
  const Environment env = {{x, 1.0}, {y, 2.0}};

  EXPECT_EQ(f1(param), e1.Evaluate(env));
  EXPECT_EQ(f2(param), e2.Evaluate(env));
}

GTEST_TEST(SymbolicCodeGenTest, Test2) {
  const Variable x{"x"};
  const Variable y{"y"};
