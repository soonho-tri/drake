...
//   EXPECT_TRUE(CheckAlreadyExpanded(pow(x_ + y_, x_ - y_)));  // (x + y)^(x -
//   y)
// }
...
//   //     2. Evaluate e and e.Expand() under multiple environments to check
//   the
//   //        correctness of expansions.
//   //     3. A expansion is a fixpoint of Expand() function. That is, a
//   expanded
...
//   // For a math function f(x) and an expression e, we need to have the
//   following
//   // property:
//   //
...
//   // NaN is considered as not expanded so that ExpressionNaN::Expand() is
//   called
...
//   const Expression e{if_then_else(x_ > y_, pow(x_ + y_, 2), pow(x_ - y_,
//   2))}; EXPECT_THROW(e.Expand(), runtime_error);
...
//   // (4x / 3) * (6y / 2) => 4xy
//   EXPECT_PRED2(ExprEqual, ((4 * x_ / 3) * (6 * y_ / 2)).Expand(), 4 * x_ *
//   y_);
...
//   {
//     LimitMalloc guard;
//     // e_expanded is already expanded, so the following line should not
//     create a
...
TEST_F(SymbolicExpansionTest, Zzz) {
  const Expression& x{x_};
  const Expression& y{y_};
  const Expression& z{z_};

  const Expression e1{1 + x + y};
  const Expression e2{2 + y + z};
  const Expression e3{3 + x + z};

  // const Expression e{(e1 + e2 / ((e3 + e1) * (e2 + e1))) * (e1 - e2 / e3) *
  //                    (e1 * e2 / e3)};

  const Expression e{(e1 + e2 / ((e3 + e1) * (e2 + e1))) * (e1 - e2 / e3) *
                     (e1 * e2 / e3)};

  std::cerr << e.Expand() << std::endl;
}
