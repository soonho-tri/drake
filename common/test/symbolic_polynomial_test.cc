...
using test::PolyNotEqual;

bool MapEqual(const Polynomial::MapType& m1, const Polynomial::MapType& m2) {
  if (m1.size() != m2.size()) {
    return false;
  }
  for (const pair<const Monomial, Expression>& item : m1) {
    const Monomial& m{item.first};
    const Expression& coeff{item.second};
    const auto it = m2.find(m);
    if (it == m2.end()) {
      // Key `m` is not in `p`.
      return false;
    }
...
TEST_F(SymbolicPolynomialTest, ConstructFromMapType1) {
  ...
  EXPECT_TRUE(MapEqual(p.monomial_to_coefficient_map(), map));
  EXPECT_PRED2(ExprEqual, p.ToExpression(), -2 * a_ * x_ + 4 * b_ * pow(y_, 3));
...
TEST_F(SymbolicPolynomialTest, ConstructFromMapType2) {
  ...
  EXPECT_TRUE(MapEqual(Polynomial{p_map}.monomial_to_coefficient_map(), p_map));
}
...
TEST_F(SymbolicPolynomialTest, ConstructFromMonomial) {
  for (int i = 0; i < monomials_.size(); ++i) {
    ...
    for (const std::pair<Monomial, Expression>& map :
      ...
      EXPECT_PRED2(ExprEqual, map.second, 1);
    }
  }
}
...
TEST_F(SymbolicPolynomialTest, ConstructorFromExpressionAndIndeterminates1) {
  ...
  EXPECT_TRUE(MapEqual(p1.monomial_to_coefficient_map(),
                       Polynomial::MapType({{Monomial{}, Expression(1.0)}})));
  // p₂ = ax + by + cz + 10
  const Polynomial p2{a_ * x_ + b_ * y_ + c_ * z_ + 10, var_xyz_};
  EXPECT_TRUE(MapEqual(p2.monomial_to_coefficient_map(),
  ...
  EXPECT_TRUE(
      MapEqual(p3.monomial_to_coefficient_map(),
               Polynomial::MapType(
                   // x²y ↦ 3ab²
  ...
  EXPECT_TRUE(
      MapEqual(p4.monomial_to_coefficient_map(),
               Polynomial::MapType(
                   {{Monomial{{{var_x_, 2}, {var_y_, 1}}}, 3 * a_ * pow(b_, 2)},
...
TEST_F(SymbolicPolynomialTest, MultiplicationPolynomialPolynomial2) {
  ...
  EXPECT_TRUE(
      MapEqual(product_map_expected, (p1 * p2).monomial_to_coefficient_map()));
}
...
TEST_F(SymbolicPolynomialTest, ConstructNonPolynomialCoefficients) {
  ...
  for (const pair<Expression, Polynomial::MapType>& item : testcases) {
    ...
    EXPECT_TRUE(MapEqual(p.monomial_to_coefficient_map(), expected_map));
  }
}
...
TEST_F(SymbolicPolynomialTest, Hash) {
  ...
  EXPECT_EQ(h(p1), h(p2));
  p1 += Polynomial{y_};
  EXPECT_PRED2(PolyNotEqual, p1, p2);
  EXPECT_NE(h(p1), h(p2));
}
