...
Matrix4<Expression> scale(const Vector3<Expression>& scale_vec) {
  ...
  m << scale_vec[0], z, z, z, z, scale_vec[1], z, z, z, z, scale_vec[2], z, z,
      z, z, o;
  return m;
}
...
Vector3<Expression> xfm_point(const Matrix4<Expression>& xform,
  ...
  Vector4<Expression> tpt(xform(0, 0) * pt[0] + xform(0, 1) * pt[1] +
...
Expression render(const Matrix4<Expression>& cam_to_world,
  ...
  Matrix4<Expression> cam_to_sample =
      scale(Vector3<Expression>(
          Expression{-0.5}, Expression{-0.5} * aspect_ratio, Expression{1.0})) *
      translate(Vector3<Expression>(
          Expression{-1.0}, Expression{-1.0} / aspect_ratio, Expression{0.0})) *
  ...
  Vector3<Expression> ray_dir = normalize(
      xfm_point(sample_to_cam, Vector3<Expression>(x, y, Expression{0.0})));
  ...
  Expression mask = if_then_else(b0 >= 0.0 && b1 >= 0.0 && b0 + b1 <= 1.0,
                                 1.0,  // hit
                                 0.0   // miss
  );
...
sensors::ImageExpr1E render(const Matrix4<Expression>& cam_to_world,
                            const Vector3<Expression>& tri_v0,
                            const Vector3<Expression>& tri_v1,
                            const Vector3<Expression>& tri_v2,
                            const Expression& z_near, const Expression& z_far,
                            const Expression& fov_y, const int img_width,
                            const int img_height) {
  Expression aspect_ratio{double(img_width) / double(img_height)};
  sensors::ImageExpr1E img(img_width, img_height);
